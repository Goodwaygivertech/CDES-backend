...
export const updateSharedDocument = async (req, res) => {
  ...
  try {
    ...
    const isEmailAlreadyShared = documentToShare.sharedWith.includes(bodyData.userEmailToShareWith);
    if (isEmailAlreadyShared) {
      return res.status(400).json({ success: false, message: 'Document is already shared with this email' });
    }

    // Update the document with the new user's email in the sharedWith array
...
export const updateDocData = async (req, res) => {
  ...
  try {
    // Check if the required fields are present in the request body
    if (!newDocData) {
      return res.status(400).json({ success: false, message: 'New document data is missing' });
    }
    ...
    return res.status(200).json({ success: true, message: 'Document data updated successfully' });
  } catch (error) {
...
export const deleteDocument = async(req, res)=> {
  ...
  try {
    ...
    return res.status(200).json({ success: true, message: 'Document deleted successfully' ,deletedDoc});
  } catch (error) {
